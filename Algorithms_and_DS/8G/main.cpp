...
struct Edge {
  Edge(Vertex from, Vertex to, int id) : from(from), to(to), id(id) {}
  Vertex from;
  Vertex to;
  int id;
};
...
class FlowNetwork {
 public:
  ...
  const std::vector<typename GraphT::EdgeT>& EdgesFromVertex(int vertex) const {
    return graph_.EdgesFromVertex(vertex);
  }

  std::vector<EdgeProperties> GetProperties() { return edges_properties_; }
...
class AncestorsCalculator : public DfsVisitor<int, Edge<int>> {
 public:
  ...
  explicit AncestorsCalculator(
      OptionalEdgeVector* edges_to_ancestor_in_dfs_tree)
      : edges_to_ancestor_in_dfs_tree_(edges_to_ancestor_in_dfs_tree) {}

  void TreeEdge(GraphT::EdgeT edge) override {
    (*edges_to_ancestor_in_dfs_tree_)[edge.to] = edge;
  }

 private:
  OptionalEdgeVector* edges_to_ancestor_in_dfs_tree_;
};
...
class EdmondsKarpAlgorithm {
 public:
  ...
  void FindMaxFlow(int english_num) {
    int max_flow = 0;
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
    std::vector<EdgeProperties> edges = network_.GetProperties();

    std::cout << max_flow << '\n';
    // std::vector<EdgeT> answer;
    for (int i = 0; i < network_.NumberVertices(); ++i) {
      for (auto edge : network_.EdgesFromVertex(i)) {
        if (edges[edge.id].flow > 0 && edge.from != network_.Source() &&
            edge.to != network_.Sink()) {
          // answer.push_back(edge);
          std::cout << edge.from << ' ' << edge.to - english_num << '\n';
        }
      }
    }
 ...
 private:
  ...
  OptionalEdgeVector AncestorsInShortestPathsTree() {
    OptionalEdgeVector ancestors(network_.NumberVertices());
...
std::vector<Edge<int>> ReadGraph(int english_num, int chinese_num,
                                 std::istream& is = std::cin) {
  std::vector<Edge<int>> edges;
  int hypothesis;
  for (int i = 1; i <= english_num; ++i) {
    is >> hypothesis;
    edges.emplace_back(0, i, 1);
    while (hypothesis != 0) {
      edges.emplace_back(i, hypothesis + english_num, 1);
      is >> hypothesis;
    }
  }
  for (int i = english_num + 1; i <= english_num + chinese_num; ++i) {
    edges.emplace_back(i, english_num + chinese_num + 1, 1);
  }
...
int main() {
  ...
  int english_num;
  int chinese_num;
  std::cin >> english_num >> chinese_num;
  int servers_num = english_num + chinese_num + 2;
  auto edges = ReadGraph(english_num, chinese_num);
  auto network = BuildNetwork(edges, servers_num);
  EdmondsKarpAlgorithm flow_searcher(network);
  flow_searcher.FindMaxFlow(english_num);
  return 0;
}