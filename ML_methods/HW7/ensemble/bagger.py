...
class Bagger:
    ...
    def fit(self, X, y) -> Bagger:
        ...
        for i in range(self.n_estimators):
            X_samp, y_samp = self.object_sampler.sample(X, y)
            feature_indices = self.feature_sampler.sample_indices(X.shape[1])
            X_samp = X_samp[:, feature_indices]
    ...
    def predict_proba(self, X) -> np.ndarray:
        ...
        prob_tensor = np.array([base_estimator.predict_proba(X[:, indices]) for base_estimator, indices in zip(self.estimators, self.indices)])
        return prob_tensor.mean(axis = 0)
